...
<form method="POST" action="{{ route('services.update', $service) }}"
      x-data="{
          ...
          rita_balance:    {{ (int) old('rita_balance', $service->rita_balance ?? 0) }},
    ...
    <div class="space-y-4">
        ...
        <div class="card space-y-3">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <label class="flex items-start gap-2.5 p-3 rounded-xl cursor-pointer border transition-colors duration-150"
                       :class="kind === 'eceran' ? 'border-primary-400 bg-primary-50 dark:bg-primary-900/20' : 'border-neutral-200 dark:border-neutral-700'">
                    <input type="radio" name="kind" value="eceran" x-model="kind" class="mt-0.5 accent-primary-600">
                    <span>
                        <span class="block text-xs font-medium text-neutral-900 dark:text-neutral-100">Eceran</span>
                        <span class="block text-[11px] text-neutral-500 mt-0.5">Harga &amp; modal diketik saat transaksi</span>
                    </span>
                </label>
                <label class="flex items-start gap-2.5 p-3 rounded-xl cursor-pointer border transition-colors duration-150"
                       :class="kind === 'rita' ? 'border-primary-400 bg-primary-50 dark:bg-primary-900/20' : 'border-neutral-200 dark:border-neutral-700'">
                    <input type="radio" name="kind" value="rita" x-model="kind" class="mt-0.5 accent-primary-600">
                    <span>
        ...
        <div class="card space-y-4" x-show="kind === 'rita'" x-cloak>
            ...
            <div>
                ...
                <p class="text-[11px] text-neutral-400 mt-1">
                    Ubah angka ini untuk top-up / koreksi saldo deposit Rita.
                </p>
            </div>
        </div>
        ...
        <div x-data="{ confirm: false }">
            <button type="button" @click="confirm = true"
                    class="btn-danger justify-center !text-xs w-full">
                <x-heroicon-o-trash class="w-3.5 h-3.5" />
                Hapus jasa
            </button>
